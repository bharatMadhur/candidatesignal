...
def persist_matches(record: dict[str, Any], matches: list[dict[str, Any]], tenant_id: str | None = None) -> None:
    ...
    with db() as conn:
        for match in matches:
            ...
            conn.execute(
                """
                insert into entity_resolution_matches (tenant_id, left_document_id, right_document_id, score, reasons)
                values (%s, %s, %s, %s, %s)
...
def list_clusters(tenant_id: str | None = None) -> list[dict[str, Any]]:
    with db() as conn:
        rows = conn.execute(
            ...
            from entity_resolution_matches erm
...
def _audit_events(match_id: str, tenant_id: str | None = None) -> list[dict[str, Any]]:
    with db() as conn:
        rows = conn.execute(
            ...
            where audit_logs.entity_type='entity_resolution_match'
...
def entity_resolution_requirements() -> dict[str, Any]:
    return {
